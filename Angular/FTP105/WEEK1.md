...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP105 `
    * `cd FTP105`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\FTP105
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP105;` and click the lightning button
  * `CREATE USER 'FTP105'@'localhost' IDENTIFIED BY 'FTP105';`
  * `GRANT ALL ON FTP105.* TO 'FTP105'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/FTP105
  * `cd restservice/canteenmanagement`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.FTP105.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/FTP105-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP105.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/FTP105/api/menu`
      * `cd ../../webui/lmApp/`
   * `npm install`
   * `ng serve`
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "FTP105"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "FTP105 Sprint 1" as label 
